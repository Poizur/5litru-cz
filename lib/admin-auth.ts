// Admin session helpers. Single shared password (`ADMIN_SECRET_KEY`) doubles
// as the HMAC signing secret for the session cookie. Web Crypto API only —
// works in Edge middleware and Node server actions.
//
// (Borrowed from olivator.cz where the same pattern has been in production.)

import { cookies } from 'next/headers'

export const COOKIE_NAME = '5litru_admin'
...
export async function createAdminSession(): Promise<void> {
  ...
  store.set(COOKIE_NAME, `${issued}.${signature}`, {
    httpOnly: true,
